...
const Faq = () => {
  ...
  const faqs = [
    ...
    },
    {
      ...
      a: "Nunc duis id aenean gravida tincidunt eu, tempor ullamcorper. Viverra aliquam arcu..."
    },
  ]
        ...
        <section id='FAQ' className='mt-[112px]'>
            <div className="container">
                <div id="Faq-Row" className='flex items-center justify-between'>
                    ...
                    {/* -----------Left Side------------ */}
                    <div className='w-[746px] border-1 border-gray300 py-[16px] rounded-[6px] box-border'>
                        {faqs.map((item, i) => (
                        <div key={i} className='border-b-1 border-gray300 py-[16px]'>
                            <div className='overflow-hidden'>
                            <div onClick={() => toggleFaq(i)} className='flex items-center justify-between px-[24px] cursor-pointer'>
                                <p className='text-gray900 font-semibold text-base'>{item.q}</p><FaAngleDown className={`duration-300 ${activeIndex === i ? 'rotate-180' : ''}`} />
                            </div>

                            <div className={`px-[24px] duration-300 overflow-hidden ${ activeIndex === i ? 'h-[200px] pt-[32px]' : 'h-0' }`}>
